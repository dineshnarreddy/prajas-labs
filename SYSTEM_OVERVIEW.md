...
| 2026-07-23 | Website v6 PUBLISHED — complete rebuild as Next.js 16 static export (from C:\prajas-labs-next): "Warm Editorial" design — Ivory #F7F7F4 + Deep Teal #0E6B57 palette (from logo network nodes) + amber #8F6A24 accents; Fraunces editorial serif display + Inter body; CSS-built WellCycle phone mockup in hero (cycle ring Day 12/28, on-device privacy badge); Proof stats strip (20 screenings / 6 domains / 5 products / 100% private); scroll-spy navbar with active teal pill; Framer Motion scroll reveals with MotionConfig reducedMotion; dark forest-ink CTA band + footer; UI/UX Pro Max audited 2x: WCAG AA contrast verified, focus rings, skip-link, scroll-margin-top 92px, inline form validation with focus management, 12px min text. Old index.html/style.css/script.js removed; _next/ assets + .nojekyll added. PRESERVED untouched: wellcycle-privacy.html, femedge-privacy.html, all product/collab pages, letterhead.html, CNAME, logos |
